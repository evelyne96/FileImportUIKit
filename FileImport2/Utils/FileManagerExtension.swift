...
extension FileManager {
    ...
    class func loadFileURLs(from folder: String, type: UTType, dir: String? = documentsDirectory) -> [URL] {
        guard let dir = dir else { return [] }
        let url = URL(fileURLWithPath: dir)
        let dataPath = url.appendingPathComponent(folder)
        ...
        do {
            let fileURLs = try FileManager.default.contentsOfDirectory(at: dataPath, includingPropertiesForKeys: nil, options: [])
            return fileURLs.filter { $0.pathExtension == type.preferredFilenameExtension }
        } catch {
            debugPrint(error.localizedDescription)
        }
